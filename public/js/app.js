...
jQuery(function ($) {
  ...
  const store = {
    ...
    getTodoById(id) {
      return this._todos.find(todo => todo.id === Number(id));
    },
    deleteTodoById(id) {
      this._todos = this._todos.filter(todo => todo.id !== Number(id));
    },
  ...
  let baseUrl = params.get('api') || 'http://localhost:8080/v1/todos';
  api.init(baseUrl);
  app.init();
